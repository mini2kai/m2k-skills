...
| `ai-delivery-hook` | AI delivery history search and manual-change detection, including `.worker_author_story/` | Read-only: no repo writes, no hook installation, no commit blocking; hard parameter caps |
| `work-orchestrator` | Full-chain orchestration with Worker Author Story handoff logs | Phase gates enforce analysis before execution; code tasks hand off to specialist skills after authorization and maintain project-level delivery story logs when triggered |
