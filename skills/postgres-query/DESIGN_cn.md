...
这个 skill 仍在使用：它是 PostgreSQL/Greenplum 的本地受控脚本方案，价值在 `sql_guard.py`、行数/超时硬上限、脱敏和审计这些代码围栏。

新的取舍不是废弃它，而是**优先配置数据库 MCP**。MCP 更适合日常只读查询、结构查看和结果返回；当没有 MCP、用户明确要求本地脚本、或需要本地围栏审计时，继续使用 `postgres-query`。
