...
```text
ai_delivery_search.py   查历史留存，给后续任务提供上下文
...
- `work-orchestrator`：Intake/Evidence 阶段可调 `ai_delivery_search.py` 取历史上下文，接手时可调 `ai_delivery_since.py` 检测人工变更；不再编排 start/prepare/finish。

## 保留的风险

- **没有强制留痕**：AI 可以完全不写交付文档。这是有意的取舍——用 commit 质量替代文档强制。如果将来变成团队多人协作、需要不可绕过的 AI 变更证据链，应该在 CI 层做，而不是回到本地 hook。
